...
<div class="sidebar-panel">
    <div class="flex h-full grow flex-col bg-white pl-[var(--main-sidebar-width)] dark:bg-navy-750">
        ...
        <div class="h-[calc(100%-4.5rem)] overflow-x-hidden pb-6" x-data="{ expandedItem: null }" x-init="$el._x_simplebar = new SimpleBar($el);">
            @foreach ($sidePanel['items'] as $key => $menuItems)
                @if ($key > 0)
                ...
                <ul class="flex flex-1 flex-col px-4 font-inter">
                    @foreach ($menuItems as $keyMenu => $menu)
                            ...
                            <li x-data="accordionItem('{{ $keyMenu }}')">
                                ...
                                <ul x-collapse x-show="expanded">
                                    @foreach ($menu['submenu'] as $keyMenu => $submenu)
                                        <li
                                            @if ($submenu['url'] === request()->fullUrl()) x-init="$el.scrollIntoView({block:'center'}); expanded = true" @endif>
                                            <a href="{{ $submenu['url'] }}"
                                               class="flex items-center justify-between p-2 text-xs+ tracking-wide
                                                 ...
                                                 {{ $submenu['url'] === request()->fullUrl() ? 'text-primary dark:text-accent-light font-medium' : 'text-slate-600 hover:text-slate-800
                                                 dark:text-navy-200
                        ...
                        @else
                            <li @if ($menu['url'] === request()->fullUrl()) x-init="$el.scrollIntoView({block:'center'});" @endif>
                                <a href="{{ $menu['url'] }}"
                                   class="flex text-xs+ py-2  tracking-wide outline-none transition-colors duration-300 ease-in-out {{ $menu['url'] === request()->fullUrl() ?
                                   'text-primary dark:text-accent-light font-medium' : 'text-slate-600  hover:text-slate-800 dark:text-navy-200 dark:hover:text-navy-50' }}">
                                    {{ $menu['title'] }}
                                </a>
                            </li>
                        @endif
